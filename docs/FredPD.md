...
- **From the field.** `call.self_initiate` (`cad.call.self_initiate`, patrol) raises a call where the officer stands — a traffic stop (from ox_target on the car, whose plate the server reads) or something seen on patrol — with the officer on it, leading, on scene; source `officer` (0029). "Attach to call" is one press and sets the unit en route; `call.attach_nearest` attaches to the nearest open call within 150 m, on scene. Status keys (no default binding): en route, on scene, available, attach to nearest, raise a call here, clear my call (`unit.progress`, `call.clear_mine`). On the MDT away from the console the card folds its dispatching section away.
...
- [M] Ten-print capture at the booking terminal (`booking.tenPrint.capture`): the officer stands at the terminal with the arrestee, both resolved and range-checked server-side exactly as a GSR swab's `targetId` is (8.3.2), and the terminal itself gated by the `accessPoint` context condition (1.4, 3.10) — the same pattern `evidence.intake` and `lab.analysis.start` already use, so this cannot be called from anywhere the officer merely claims to be at one. Files a reference entry and, the first time this character is fingerprinted at all, seeds the hidden `fpd_biometrics` row 8.1 describes — nothing else seeds it yet (DNA-only characters stay unseeded until something does, a separate, disclosed gap). Also fills in `fpd_persons.identifier` the first time this exact character is identified, refusing as a conflict rather than overwriting if the booking's person already carries a different one, and re-checking after a no-op write rather than trusting a check made before it: two officers (or two different resolved targets) racing the same booking's first identification must not let the loser file a print under the winner's now-established identity.
- [M] The fingerprint scanner's second action (the first is 3.10's fast trace collection): a live identity scan, `forensics.identity.scan`. Answers whether the person standing at the scanner already has a ten-print reference on file — an identifier equality check, not a comparison, since the server already knows exactly who a live character is. Refuses on anyone not currently held under an open custody chain: this discloses an identity, which sits behind a procedural condition here the same as everywhere else in this suite, not an on-duty officer's discretion alone.
- [S] Ballistic correlations link unsolved scenes that share a weapon.
- **Permissions:** `forensics.identity.scan`, alongside `booking.intake` for the capture side (8.8 is the scanner's second action, sharing the placement kind and the permission this suite already uses for everything else done at intake).
...
| Dispatch | `cad.call.create`, `cad.call.dispatch`, `cad.call.self_assign`, `cad.call.self_initiate`, `cad.call.clear`, `cad.call.note`, `cad.call.link`, `cad.unit.manage`, `cad.unit.status`, `cad.emergency`, `cad.broadcast`, `alpr.read.view`, `alpr.hotlist.manage` |
| Forensics | `forensics.scene.create`, `forensics.scene.release`, `forensics.evidence.collect`, `forensics.tools.use`, `forensics.identity.scan` |
