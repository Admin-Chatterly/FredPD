<script lang="ts">
  ...
  /** Surname first, as every other person list in the register (`Records.personName`). */
  function name(person: PopulationPerson): string {
    return [person.lastName, person.firstName].filter(Boolean).join(', ') || t('records.population.noName');
  }
...
{#snippet block(kind: 'persons' | 'vehicles', count: number)}
  {#if count > 0}
    <section class="border border-[var(--color-border)] px-3 py-2 text-xs" aria-labelledby="{titleId}-{kind}">
      <h2 id="{titleId}-{kind}" class="text-sm font-semibold">{t(`records.population.${kind}Title`)}</h2>
      ...
      <ul class="mt-1">
        ...
        {:else}
          {#each vehicles as vehicle (vehicle.ref)}
            <li class="flex items-center justify-between gap-3 border-t border-[var(--color-border)] py-1 first:border-t-0">
              <span>
                <span class="text-[var(--color-ink-muted)]">{t('records.population.vehicle')}</span>
                <span class="ml-2 font-[family-name:var(--font-mono)]">{vehicle.plate}</span>
              </span>
              <button
                type="button"
                class={button}
...
{/if}
{@render block('persons', persons.length)}
{@render block('vehicles', vehicles.length)}
