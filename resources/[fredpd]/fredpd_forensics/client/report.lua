...
---
--- Kinds not listed are not reportable from this client, because `afford`
--- refuses anything without an entry before a call is made. So this table is
--- the real list of what this resource can observe, and it has to carry every
--- kind the route accepts that a client is meant to be able to send.
---
--- `blood` and `bullet` are the two the route deliberately does not accept from
--- here at all: the server builds both from its own `weaponDamageEvent` pair,
--- and a client that could ask for them could put anybody's blood anywhere. See
--- the damage section in `sensors.lua`.
---
--- `item_use` and `tool` are accepted by the route and are listed here, but no
--- sensor in this resource fires them: the moments are somebody else's -- an
--- item being consumed, a lock being picked -- and they arrive through the
--- `itemUsed` and `toolUsed` exports at the bottom of `sensors.lua`, the way a
--- door arrives through `surfaceTouched`. Without a bridge calling those, touch
--- DNA and tool marks are generated by nothing; that is a wiring gap and not a
--- decision, unlike the two above.
local BUDGETS <const> = {
    ...
    -- Eating, drinking, smoking (route limit: 15 per 30 s). Consuming
    -- something is seconds of animation, so the gap is a second and a half and
    -- the ceiling is never the thing a player notices.
    item_use = { per = 12, window = 30000, minGap = 1500 },
    -- Lockpicks and crowbars (route limit: 20 per 30 s). A door at a time.
    tool = { per = 16, window = 30000, minGap = 750 },
}
