...
route.subject({
    ...
    handler = function(subject, input)
        ...
        if repo.countToday(subject.agencyId, subject.identifier) >= (tonumber(config().perDay) or 5) then
            return route.refuse(FredPD.ErrorCode.CONFLICT, { _input = 'too_many' })
        end
        ...
        announce(subject.agencyId, report.kind, row.number)
...
route.define({
    ...
    handler = function(session, input)
        return { reports = repo.inbox(session.agencyId, readableKinds(session), input.status, input.limit or 50) }
    end,
})
